...
public partial class BattleManager : Node
{
    ...
    private void Init()
    {
        _battle.Initialized += (_, _) =>
        {
            ...
            for (int i = 0; i < _battle.SecondTeam.Count; i++)
                CreateEnemyPresenterFor(_battle.SecondTeam[i], _rightSpawnPositions[i], true);
        };
    ...
    private void CreatePresenterFor(Character character, Node2D position, bool flip = false)
    {
        ...
        private void CreateEnemyPresenterFor(Character character, Node2D position, bool flip = false)
    {
        if (_enemyScene == null)
            throw new InvalidOperationException("Enemy scene isn't set.");

        var presenter = _enemyScene.Instantiate<CharacterPresenter>();
        position.AddChild(presenter);
        presenter.Init(character, flip);
    }
}
